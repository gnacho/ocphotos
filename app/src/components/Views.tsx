...
function Grid({ assets, empty }: { assets: PhotoAsset[]; empty: string }) {
  ...
  if (assets.length === 0)
    return <div className="flex h-64 items-center justify-center text-neutral-500">{empty}</div>
  return (
    <div className="grid grid-cols-3 gap-[3px] p-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
      {assets.map((a, i) => (
        <button key={a.id} onClick={() => openViewer(assets, i)} className="group relative aspect-square overflow-hidden bg-neutral-800">
...
export function Recuerdos() {
  ...
  return (
    <div className="space-y-8 p-4 pb-24">
      <h2 className="flex items-center gap-2 px-1 text-lg font-medium text-neutral-200">
          ...
          <section key={year}>
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 text-left">
                ...
                <p className="text-sm text-neutral-300">{list.length} {list.length === 1 ? 'foto' : 'fotos'}{list[0].place ? ` · ${list[0].place}` : ''}</p>
              </div>
            </button>
            <div className="grid grid-cols-4 gap-[3px] md:grid-cols-8">
              {list.slice(1, 9).map((a, i) => (
                <button key={a.id} onClick={() => openViewer(list, i + 1)} className="aspect-square overflow-hidden rounded-md bg-neutral-800">
  ...
  return (
    <div className="p-4 pb-24">
      <div className="mb-4 flex items-center justify-between px-1">
        <h2 className="flex items-center gap-2 text-lg font-medium text-neutral-200"><Users size={18} /> Personas</h2>
        ...
        {named.map((p) => (
          ...
          >
            ...
            <div className="text-center">
              <p className="text-sm text-neutral-200">{p.name}</p>
              <p className="text-xs text-neutral-600">{p.count}</p>
            </div>
          </button>
        ))}
      </div>
...
export function Albums() {
  const { albums, assets, openViewer } = useStore()
  return (
      ...
      {albums.map((al) => {
        ...
        return (
          <button key={al.id} onClick={() => openViewer(list, 0)} className="group text-left">
            <div className="relative mb-2 aspect-square overflow-hidden rounded-xl bg-neutral-800">
            ...
            <p className="font-medium text-neutral-200">{al.name}</p>
            <p className="text-xs text-neutral-600">{new Intl.DateTimeFormat('es', { month: 'long', year: 'numeric' }).format(al.createdAt)}</p>
          </button>
        )
      })}
    </div>
    ...
    <div className="grid grid-cols-2 gap-5 p-4 pb-24 md:grid-cols-3 lg:grid-cols-4">
      {folders.map(([dir, list]) => (
        <button key={dir} onClick={() => openViewer(list, 0)} className="group text-left">
          <div className="relative mb-2 aspect-[4/3] overflow-hidden rounded-xl bg-neutral-800">
          ...
          <p className="truncate font-medium text-neutral-200">{dir.replace(/^\//, '')}</p>
          <p className="text-xs text-neutral-600">{list.length} elementos</p>
        </button>
      ))}
    </div>
  )
...
export function Favoritos() {
  ...
  return (
    <div>
      <h2 className="flex items-center gap-2 p-4 pb-0 text-lg font-medium text-neutral-200">
